...
Use these settings to configure iOS/iPadOS devices to run specific apps in autonomous single app mode. When this mode is configured, and the user starts one of the configured apps, the device is locked to that app and app/task switching is disabled until the user exits the allowed app. For example, in a school or university environment add an app that lets users take a test on the device. Or locking the device into the Company Portal until the end user authenticates. When the apps actions are completed by the user, or you remove this policy, the device returns to its normal state.
