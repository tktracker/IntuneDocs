...
Entries that resemble the following are logged in the devices OMADM log when the device gets the profile from Intune:
...
### Validate that the iOS or iPadOS device was sent the policy
...
Entries that resemble the following are logged in iOS debug log when the device gets the profile from Intune:
